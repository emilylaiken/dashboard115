...
def generateFigures(chart_sql, total_sql, avg_sql, title):
    ...
    chart = chartmaker.calls_by_day(dates, numcalls, title)
    # Total
...
def generateFiguresDownload(chart_sql, total_sql, avg_sql, title, filename, numdays):
    con = sqlite3.connect("logs115.db")
    cur = con.cursor()
    cur.execute(chart_sql)
    calls_by_date = cur.fetchall()
    ...
    for log in calls_by_date:
        dates = dates + [log[0]]
        numcalls = numcalls + [log[1]]
    chart = chartmaker.calls_by_day_offline(dates, numcalls, title, filename)
    # Total
    cur.execute(total_sql)
    totals = cur.fetchall()
    total = totals[0][0]
...
def addFigure(condition_string, table, title, charts, totals, averages):
    ...
    chart, total, avg = generateFigures(chart_sql, total_sql, avg_sql, title + title_addon)
    charts.append(chart)
    totals.append(total)
    averages.append(avg)
...
def addHCReportChart(condition_strings, title):
    ...
    for condition_string in condition_strings:
        cur.execute("SELECT week_id, sum(" + condition_string + ") FROM calls JOIN hc_reports ON calls.call_id = hc_reports.call_id WHERE date >= " + "'" + starting_date_string + "'" + " AND date <= " + "'" + ending_date_string + "'" + " GROUP BY week_id;")
